...
test('Pages build receives only public Supabase Vite variables', async () => {
  ...
  assert.match(buildStep, /test\s+-n\s+"\$\{VITE_SUPABASE_URL:-\}"/)
  assert.match(buildStep, /test\s+-n\s+"\$\{VITE_SUPABASE_PUBLISHABLE_KEY:-\}"/)
  ...
  assert.doesNotMatch(buildStep, /echo\s+.*VITE_SUPABASE_(?:URL|PUBLISHABLE_KEY)=/i)
})
